...
from conduit.workers.jobs import probe_providers, refresh_route_stats, rollup_usage
...
async def startup(ctx: dict[str, Any]) -> None:
    ...
    ctx["route_stats"] = RedisRouteStats(redis_client)
...
class WorkerSettings:
    functions: ClassVar[list[Any]] = [probe_providers, refresh_route_stats, rollup_usage]
    cron_jobs: ClassVar[list[Any]] = [
        cron(probe_providers, second=0, run_at_startup=True),  # every minute
        cron(refresh_route_stats, second=0, run_at_startup=True),  # every minute
